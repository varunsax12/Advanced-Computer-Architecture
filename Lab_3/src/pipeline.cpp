...
void pipe_cycle_issue(Pipeline *p)
{
    ...
    // VARUN TODO: For superscalar, when issuing, find the youngest across all lanes

    int pipeIndex[(int)PIPE_WIDTH];
    for (unsigned int i = 0; i < PIPE_WIDTH; i++)
    {
        pipeIndex[i] = i;
    }
    ...
    for (unsigned int i = 0; i < PIPE_WIDTH; i++)
    {
        ...
        {
            if (p->ID_latch[j+1].inst.inst_num < p->ID_latch[j].inst.inst_num)
            {
                std::swap(pipeIndex[j], pipeIndex[j+1]);
            }
        }
    }
    ...
    for (unsigned int j = 0; j < PIPE_WIDTH; j++)
    {
        ...
        {
            // space exists in the rob
            InstInfo idInstr = p->ID_latch[i].inst;

            // check if src1_needed
            if (idInstr.src1_reg != -1)
            {
                // check if RAT has any mapping for it
                if (p->rat->entries[idInstr.src1_reg].valid == false)
                {
                    // set the src to ready
                    idInstr.src1_ready = true;
                }
                else
                {
                    // rat has a mapping
                    idInstr.src1_tag = rat_get_remap(p->rat, idInstr.src1_reg);
                    if (idInstr.src1_tag == -1)
                    {
                        std::cout << "Developed error 1" << std::endl;
                    }
                    // check if the inst in rob is ready
                    if (rob_check_ready(p->rob, idInstr.src1_tag))
                    {
                        idInstr.src1_ready = true;
                    }
                }
            ...
            {
                // if source not needed => always ready
                idInstr.src1_ready = true;
            }

            // check if src2_needed
            if (idInstr.src2_reg != -1)
            {
                // check if RAT has any mapping for it
                if (p->rat->entries[idInstr.src2_reg].valid == false)
                {
                    // set the src to ready
                    idInstr.src2_ready = true;
                }
                else
                {
                    // rat has a mapping
                    idInstr.src2_tag = rat_get_remap(p->rat, idInstr.src2_reg);
                    if (idInstr.src2_tag == -1)
                    {
                        std::cout << "Developed error 1" << std::endl;
                    }
                    // check if the inst in rob is ready
                    if (rob_check_ready(p->rob, idInstr.src2_tag))
                    {
                        idInstr.src2_ready = true;
                    }
                }
            ...
            {
                // if src2 not needed => always ready
                idInstr.src2_ready = true;
            }

            // check if dest_needed
            if (idInstr.dest_reg != -1)
            {
                // set tag for inst destination reg to the head ptr of the rob
                idInstr.dr_tag = p->rob->tail_ptr;
                // update the rat
                rat_set_remap(p->rat, idInstr.dest_reg, idInstr.dr_tag);

            }
            // insert the updated instruction in the rob
            rob_insert(p->rob, idInstr);
            p->ID_latch[i].valid = false;
        }
        else
        {
            // no space exists in the rob
        }
    }
}
...
void pipe_cycle_schedule(Pipeline *p)
{
    ...
    for (unsigned int j = 0; j < PIPE_WIDTH; ++j)
    {
        ...
        if (SCHED_POLICY == SCHED_IN_ORDER)
        {
            ...
            // Stop conditions
            // 1. Checked the whole rob
            // 2. SC latch filled
            // 3. Found inst not ready


            // i => increment from current head location
            for (int i = 0; i < (int)NUM_ROB_ENTRIES; ++i)
            {
                int currRobIndex = (p->rob->head_ptr + i) % NUM_ROB_ENTRIES;
                // check if inst valid
                if (p->rob->entries[currRobIndex].valid == false)
                {
                    // if inst invalid => cannot choose
                    continue;
                }
                // check if inst exec
                if (p->rob->entries[currRobIndex].exec == true)
                {
                    // if inst executing => cannot choose
                    continue;
                }

                // => inst is valid and not executing
                // check if inputs are ready
                if (rob_check_inputs_ready(p->rob, currRobIndex))
                {
                    // => inst can be scheduled
                    robIndex = currRobIndex;
                    break;
                }
                else
                {
                    // => inst not ready => stop checking inst (Stop condition 1)
            ...
            {
                // stop condition 2 not met => required entry found
                // => inst ready to exec
                rob_mark_exec(p->rob, p->rob->entries[robIndex].inst);
                // add to the SC latch for executing
                p->SC_latch[j].inst = p->rob->entries[robIndex].inst;
...
void pipe_cycle_writeback(Pipeline *p)
{
    ...
    for (unsigned int j = 0; j < MAX_WRITEBACKS; ++j)
    {
        // check if inst valid and writing to reg
        if (p->EX_latch[j].valid && p->EX_latch[j].inst.dest_reg != -1)
        {
            rob_wakeup(p->rob, p->EX_latch[j].inst.dr_tag);
        }
        rob_mark_ready(p->rob, p->EX_latch[j].inst);
        p->EX_latch[j].valid = false;
    }
...
void pipe_cycle_commit(Pipeline *p)
{
    ...
    for (unsigned int j = 0; j < PIPE_WIDTH; ++j)
    {
        ...
        {
            // store the prf id
            InstInfo toCommit = rob_remove_head(p->rob);
            pipe_commit_inst(p, toCommit);

            // check if rat mapping exists
            if (toCommit.dest_reg != -1)
            {
                // check if the prfID is the same as the one in the rat
                if ((int)p->rat->entries[toCommit.dest_reg].prf_id == toCommit.dr_tag)
                {
                    rat_reset_entry(p->rat, toCommit.dest_reg);
                }
            }
        }
    }
