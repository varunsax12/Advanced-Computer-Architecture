...
void rob_print_state(ROB *rob)
{
    ...
    printf("Number of ROB entries -> %d\n", NUM_ROB_ENTRIES);
    printf("Entry ::  Inst\t   Valid  Ready  Exec src1_ready src1_reg src2_ready src2_reg dest_reg\n");
    for (unsigned int i = 0; i < NUM_ROB_ENTRIES; i++)
    {
        printf("%5d ::  %d\t", i, (int)rob->entries[i].inst.inst_num);
        printf(" %5d\t", rob->entries[i].valid);
        printf(" %5d\t", rob->entries[i].ready);
        printf(" %5d\t", rob->entries[i].exec);
...
bool rob_check_space(ROB *rob)
{
    ...
    // Write possible if tail and head pointer not pointing to the same location
    // and the current location is valid as when head==tail could also mean that
    // the rob is empty
    return rob->head_ptr != rob->tail_ptr || rob->entries[rob->tail_ptr].valid == false;
}
...
int rob_insert(ROB *rob, InstInfo inst)
{
    ...
    {
        // ROB has space so add the enty
        ROBEntry newEntry;
        newEntry.valid = true;
        newEntry.exec = false;
        newEntry.ready = false;
        newEntry.inst = inst;
        rob->entries[rob->tail_ptr] = newEntry;

        // increment the tail pointer
        int currentIndex = rob->tail_ptr;
...
/*
* Function to return the index of the rob entry
* which contains the inst
* 
* @param rob -> pointer to the ROB structure
* @param inst -> inst to find
*/
int find_rob_inst(ROB* rob, InstInfo inst)
{
    int robIndex = -1;
    for (unsigned int i = 0; i < NUM_ROB_ENTRIES; ++i)
    {
...
void rob_mark_exec(ROB *rob, InstInfo inst)
{
    ...
    int robIndex = find_rob_inst(rob, inst);
    rob->entries[robIndex].exec = true;
}
...
void rob_mark_ready(ROB *rob, InstInfo inst)
{
    ...
    // TODO: Update that entry.
    int robIndex = find_rob_inst(rob, inst);
    rob->entries[robIndex].ready = true;
}
...
bool rob_check_ready(ROB *rob, int tag)
{
    ...
    return rob->entries[tag].ready && rob->entries[tag].valid;
}
...
InstInfo rob_remove_head(ROB *rob)
{
    ...
    if (rob_check_head(rob))
    {
        ...
        InstInfo currInst = rob->entries[rob->head_ptr].inst;
    ...
    {
        InstInfo nopInst;
        // return nop when the head is not ready
        return nopInst;
    }
}

/*
* If the rob entry has both its src1 and src2 set to ready, the
* return true
*
* @param rob the ROB
* @return whether both sources are ready
*/
bool rob_check_inputs_ready(ROB* rob, int tag)
{
    return (rob->entries[tag].inst.src1_ready && rob->entries[tag].inst.src2_ready);
}
